...
class moonApi(Resource):
    """
    This class represent an API for moon phase
    """
    ...
    def get(self):
        ...
        logging.log(logging.INFO, strings.LOG_API_MOON, user.user_name)

        moon = MoonPhase()

        moon_data = {"moon_phase": moon.phase_text}
...
def phase(phase_date=DateTime.now()):
    ...
    day = get_julian_datetime(phase_date) - c.epoch
    '''
    if hasattr(phase_date, "jdn"):
        day = phase_date.jdn - c.epoch
    else:
        day = phase_date.timestamp() - c.epoch'''
